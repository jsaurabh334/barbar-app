...
class NotificationsScreen extends StatefulWidget {
  ...
  const NotificationsScreen({super.key, this.role});
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Color _colorForType(String type) {
    ...
    if (type.startsWith('order')) return AppColors.warning;
  ...
  void _onTapNotification(NotificationModel notif) {
    ...
    if (notif.link != null && notif.link!.isNotEmpty) {
      _handleDeepLink(notif.link!);
    }
  }

  void _handleDeepLink(String link) {
    final uri = Uri.tryParse(link);
    if (uri == null || uri.scheme != 'barbar') return;
    final segments = uri.pathSegments;
    if (segments.isEmpty) return;
    final module = segments.first;

    if (module == 'booking' && segments.length > 1) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Opening booking ${segments[1]}...'), backgroundColor: AppColors.primary),
      );
    } else if (module == 'order' && segments.length > 1) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Opening order ${segments[1]}...'), backgroundColor: AppColors.primary),
      );
    } else if (module == 'wallet') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Opening wallet...'), backgroundColor: AppColors.primary),
      );
    }
  }
            ...
            if (state.notifications.isEmpty) {
              return Center(
                child: Column(
                  ...
                  children: [
                    const Icon(LucideIcons.bellOff, size: 64, color: AppColors.textMuted),
